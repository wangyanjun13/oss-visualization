...
class Analysis(object):
    ...
    def generate_sensor_data(self, frames, trans_matrix, intrinsic, sensors, occ):
        ...
        all_resources = {}
        sensor_data_format = {}
        clipid2occ_path_dict = {}
        logger.info(f"len frames: {len(frames)}")
        for frame in frames: 
            occ_path = frame.get("occ_path")
            clip_id = frame.get("clip_id", "") + "_" +str(frame["frame_id"])
            clipid2occ_path_dict[clip_id] = occ_path
            
            if occ and not occ_path: # 处理的是occ数据，但是没有occ_path continue
                continue 
            ...
            for key, value in sensor_data.items():
                ...
                if not value:
                    raise Exception(f"frame_id: {frame['frame_id']}, key: {key}, value: {value} error")
        ...
        if occ:
            pkl_path_list = list(clipid2occ_path_dict.values())
            path_to_result = occ_utils.multi_thread_compress_occ_data(pkl_path_list)
            
            for clip_id, pkl_path in tqdm.tqdm(clipid2occ_path_dict.items(), desc="write pkl data"):
                file_name = path_to_result[pkl_path]
                # file_name = str(time.time())
                # oss_path = os.path.join(RADAR_DIR, file_name+".json")
                # with refile.smart_open(oss_path, "w") as f:
                #     f.write(json.dumps(pkl_result))
                
                oss_url = f'{BASE_URL}/{GET_RADAR_URI}/{file_name+".json"}'
                all_resources[clip_id].append({
                    "key": "occ",
                    "ext": "json",
                    "oss_url": oss_url
                })
    ...
    def process_frames(self, _id, frames, file_path, trans_matrix, calibrated_sensors, intrinsic, sensors, is_project, is_last=False, occ=False):
        all_resources = self.generate_sensor_data(frames, trans_matrix, intrinsic, sensors, occ)
        ...
        if is_project:
            ...
            for frame in tqdm.tqdm(frames, desc="insert frame"):
                ...
                if old_resources:
                    ...
                    sensors = [item["key"] for item in resources if item["key"] != "occ"]
                    final_sensors = set(sensors)
                
                final_sensors = list(final_sensors)
                # if sensors and len(sensors) != len(final_resouces):
                #     print(f"sensors: {sensors}, final_sensors: {final_sensors}")
    ...
    def analysis_file(self, file_path: str, sensors: list, occ: bool = False, is_project: bool = False):
        """
        解析pkl或者json文件
        @file_path 文件路径
        @sensors 选择的sensor列表
        ...
        try:
            ...
            if file_path.endswith(".pkl"):
                ...
                if data:
                    ...
                    self.process_frames(_id, frames, file_path, trans_matrix, calibrated_sensors, intrinsic, sensors, is_project, is_last=True, occ=occ)
...
if __name__ == '__main__':
    ...
    file_path = "s3://broadside-transform/dev/temp/20250306/test1.json"
    # file_path = "s3://e2e-rhea-data/static_od_prelabel/car_z11/ppl_bag_20250131_160407_det/v0_250205_155435/0107/occ_data/occ_dump/tf-rhea-data-bpp/new_track_gjh/prelabel/0107.json"
    sensors = ["middle_lidar","cam_front_120","cam_front_30"]
    analysis.analysis_file(file_path, sensors, occ=True, is_project=False)
